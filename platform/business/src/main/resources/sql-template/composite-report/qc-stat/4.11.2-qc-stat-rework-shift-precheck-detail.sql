    ...
    <#if reworkCode??>
     ...
     s1 AS(
         ...
         FROM pre_check_record
                  INNER JOIN
              wheel_record
              ON pre_check_record.wheel_serial = wheel_record.wheel_serial
         WHERE
                 ope_d_t > :beginDate
           AND
                 ope_d_t <= :endDate
...
FROM
    t1
        INNER JOIN pre_check_record ON t1.wheel_serial = pre_check_record.wheel_serial
        INNER JOIN s1 ON pre_check_record.inspector_id = s1.inspector_id
WHERE
        pre_check_record.ts = 1
GROUP BY
    pre_check_record.inspector_id,
