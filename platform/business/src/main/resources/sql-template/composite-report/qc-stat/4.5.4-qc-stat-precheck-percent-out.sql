WITH a AS (
    SELECT wheel_record.out_sourcing_dep AS out_company,
           CONVERT(VARCHAR (10), ope_d_t, 120) AS                                   ope_d_t,
           cope_inspector_id,
           COUNT(pre_check_record.wheel_serial) AS                                  amount,
           SUM(CASE WHEN pre_check_record.rework_code = '' THEN 1 ELSE 0 END) AS    no_rework_amount,
           SUM(
                   CASE WHEN pre_check_record.rework_code = '' AND pre_check_record.scrap_code = '' THEN 1 ELSE 0 END) AS no_rework_scrap_amount,
           SUM(CASE
                   WHEN pre_check_record.rework_code IN ('3E', '3R', '4E', '4R', '5E', '5R', '6E', '6R') THEN 1
                   ELSE 0 END) AS                                                   er3456,
           SUM(CASE WHEN pre_check_record.rework_code IN ('3E', '3R') THEN 1 ELSE 0 END) AS              er3,
           SUM(CASE WHEN pre_check_record.rework_code IN ('4E', '4R') THEN 1 ELSE 0 END) AS              er4,
           SUM(CASE WHEN pre_check_record.rework_code IN ('5E', '5R') THEN 1 ELSE 0 END) AS              er5,
           SUM(CASE WHEN pre_check_record.rework_code IN ('6E', '6R') THEN 1 ELSE 0 END) AS              er6
    FROM pre_check_record
             INNER JOIN wheel_record ON pre_check_record.wheel_serial = wheel_record.wheel_serial
    WHERE
        wheel_record.out_sourcing_dep IS NOT NULL
      AND ts = 1
      AND ope_d_t >= :beginDate
      AND ope_d_t < :endDate
    GROUP BY wheel_record.out_sourcing_dep,
