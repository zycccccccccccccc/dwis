...
@Service
@Slf4j
public class StockReportExportService {

  private static final String TEMPLATE_NAME = "2.4-stock-report.xlsx";
  ...
  public void export(Map<String, Object> parameterMap,
    ...
    JSONObject stat = multiReportProdStatService.stockReport(parameterMap);
    ...
    try (InputStream inputStream = resource.getInputStream();
      ...
      // 标题
      sheet.getRow(0).getCell(0).setCellValue("成品入库报告");
      ...
      for (int i = 0; i < resultList.size(); i++) {
        ...
        CellRangeAddress region = new CellRangeAddress(currentRow, currentRow, 1, 2); // 2个total合并
        sheet.addMergedRegion(region);
        region = new CellRangeAddress(currentRow - dataList.size(), currentRow, 0, 0); // 最左侧design合并
        sheet.addMergedRegion(region);
        currentRow++;
      }
      fillRow(sheet, 4, currentRow, stat.getJSONObject("total"));
      CellRangeAddress region = new CellRangeAddress(currentRow, currentRow, 0, 2);
      sheet.addMergedRegion(region);
  ...
  private void fillRow(Sheet sheet, int templateRow, int currentRow, JSONObject data) {
    ...
    row.getCell(column++).setCellValue(data.getString("checkCode"));
