...
@Service
@Slf4j
public class StatByModelExportService {
    ...
    for (int i = 0; i < dataArray.size(); i++) { // 一层
      ...
      for (int j = 0; j < level2DataArray.size(); j++) { // 二层
        ...
        ExcelUtil.copyCell(templateSheet, 0, 0,
          ...
          2, 13);
        currentSheet.addMergedRegion(new CellRangeAddress(currentRow, currentRow + 1, 2, 2));
        currentRow += 2;
  ...
  private void copyData(Sheet sheet, JSONArray dataArray) {
    ...
    for (int i = 0; i < dataArray.size(); i++) { // 一层
      ...
      for (int j = 0; j < leve2DataArray.size(); j++) { // 二层
        ...
        sheet.getRow(currentRow).getCell(2).setCellValue("合计"); // 合计
        fileStatData(sheet, currentRow, 3, leve2Data); // 合计部分
        currentRow += 2;
